...
class NewDetails extends Component {
    ...
    componentDidMount() {
        axios.post("http://localhost:6600/News/news", { id: this.props.match.params.id })
            .then(res => {
                this.setState({
                    data: [res.data]
                })
                                ...
                                <div className='contact_divider'>
                                    <div className="contact_data main_para2 ">
                                        <Card className="m-2" key={index}>
                                            <Card.Title className="pt-5 pl-5 text-left lead text-info"><h2>{value.NewsTitle}</h2></Card.Title>
                                            <Card.Body >
                                                <Card.Img variant="top" className="h-25" src={value.url} />
                                                <Card.Text className="mt-5 mb-5 text-left">
                                                    {value.NewsText}<br></br>
                                                    <div className="mt-2 small">
                                                        {value.NewsDec}
                                                    </div>
                                                </Card.Text>
                                                <div className="text-left mt-5">
                                                    <Link to="/News"><Button variant="danger">Go Back</Button></Link>
                                                </div>
                                            </Card.Body>
                                            <Card.Footer>
